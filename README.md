# architecture-sprint-5
## Задание 1. Настроить виртуалку
[Задание 1. terraform config для виртуалки](terraform/main.tf)
## Задание 2. Развернуть веб-ассистента и настроить RASA, адаптировав на русский язык
[Задание 2. Логи тестового взаимодействия](rasa/rasa_out.log)
## Доп задание. Развернуть веб-ассистента и настроить RASA
[Доп задание. terraform config](terraform/main.tf)

[Новое состояние с CPU=4](terraform/terraform.tfstate)

![Новое состояние с CPU=4 в консоли управления Yandex Cloud](terraform/final_vm.jpg)

[Старое состояние с CPU=2](terraform/terraform.tfstate.backup)